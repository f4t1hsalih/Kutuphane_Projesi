...
namespace Kutuphane_Projesi
{
    public partial class FormKutuphaneIstatistik : Form
    {
        ...
        private void FormKutuphaneIstatistik_Load(object sender, EventArgs e)
        {
            ...
            SqlCommand komut = new SqlCommand("select K.KutuphaneAdi, Sum(Adet) from Kutuphane_Kitap Kk inner join Kutuphane K on Kk.KutuphaneId=K.KutuphaneId group by K.KutuphaneAdi", bag.baglanti());
            SqlDataReader dr = komut.ExecuteReader();
            while (dr.Read()) chart1.Series["Kütüphane"].Points.AddXY(dr[0], dr[1]);

            SqlCommand komut1 = new SqlCommand("select count(KutuphaneAdi) from Kutuphane", bag.baglanti());
            dr = komut1.ExecuteReader();
            while (dr.Read()) labelSorgu1.Text = dr[0].ToString();

            SqlCommand komut2 = new SqlCommand("Select Count(distinct KutuphaneAdresilid) from Kutuphane", bag.baglanti());
